...
bool CheckSpendGravityCoinTransaction(const CTransaction &tx,
	...
	LogPrintf("CheckSpendGravityCoinTransaction denomination=%d nHeight=%d\n", targetDenomination, nHeight);
	...
	if(tx.IsZerocoinSpend()) {
		...
		BOOST_FOREACH(const CTxOut &txout, tx.vout)
		{
			if (!isVerifyDB) {
                ...
                case libzerocoin::ZQ_PEDERSEN*COIN:
                case libzerocoin::ZQ_WILLIAMSON*COIN:
                    if(!CheckSpendGravityCoinTransaction(tx, (libzerocoin::CoinDenomination)(txout.nValue / COIN), state, hashTx, isVerifyDB, nHeight, isCheckWallet, zerocoinTxInfo))
                        return false;
                    break;
